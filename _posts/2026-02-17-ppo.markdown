...
*This is the second post in the series "RL for Language Models." The [first post]({% post_url 2026-02-16-reinforce %}) derived the REINFORCE gradient estimator and introduced baselines and the advantage. This post covers the two additions that made RL practical for language models: clipping and the critic.*

---

The [previous post]({% post_url 2026-02-16-reinforce %}) gave us REINFORCE: sample a response, score it, push the model toward high-scoring responses and away from low-scoring ones. The math is clean, the algorithm is simple, and it's the foundation of everything that follows.
...
5. [Post 1 in this series]({% post_url 2026-02-16-reinforce %}). The REINFORCE algorithm, baselines, and the advantage.
